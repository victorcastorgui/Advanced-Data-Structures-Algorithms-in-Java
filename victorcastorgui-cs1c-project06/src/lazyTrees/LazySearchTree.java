...
public class LazySearchTree<E extends Comparable< ? super E > >
    ...
    /**
     * This is a boolean method contructor,
     * It removes all soft deleted nodes under the tree nodes physically
     * @return true/false base on return.
     */
    public boolean collectGarbage()
    {
        LazySTNode root;
        root = collectGarbage(mRoot);
        mRoot = root;

        return(mSizeHard == mSize);
    }

    /**
     * This is a protected method. It removes all soft deleted nodes physically.
     * @param root the node to be garbage collected.
     * @return the garbage collected
     */
    ...
    /**
     * this method has the same signature as that of remove() which does lazy deletion.
     * The difference is that now we are explicitly hard removing nodes from the tree.
